...
규칙:
- 반드시 질문을 딱 1개만 하세요. 절대 2개 이상 묻지 마세요.
- 회의는 지금 바로 시작할 것이므로 "언제 할 건지", "참석자 일정", "회의 시간" 같은 질문은 절대 하지 마세요.
- 이미 작성된 내용(목적/배경/결과물)에서 의사결정에 실질적으로 영향을 미치는 빠진 정보를 질문하세요.
- 좋은 질문 예시: 의견 차이가 있는 핵심 쟁점, 결정 기준, 미리 합의된 제약 조건, 이해관계자 입장 차이
- 나쁜 질문 예시: 회의 일시, 참석자 이름, 회의 장소, 이미 작성된 내용 재확인
    ...
    <div className="card max-w-xl mx-auto space-y-5">
      ...
      {members.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {members.map((m) => (
            <span key={m.id} className="px-2.5 py-1 rounded-full text-sm font-medium bg-accent/10 text-accent border border-accent/20">
              {m.name}
            </span>
          ))}
        </div>
        ...
        {FIELDS.map((f) => (
          <div key={f.key}>
            <div className="flex items-baseline gap-1.5 mb-1.5">
              ...
              <span className="text-base font-semibold text-slate-200">{f.label}</span>
      ...
      <div className="rounded-xl border border-accent/20 bg-accent/5 px-4 py-4 space-y-3">
        ...
        <div>
          ...
          <button className="btn-secondary w-full text-sm py-2 disabled:opacity-40"
          ...
          {suggestError && <p className="text-xs text-red-400 text-center">{suggestError}</p>}
          {suggestion && (
